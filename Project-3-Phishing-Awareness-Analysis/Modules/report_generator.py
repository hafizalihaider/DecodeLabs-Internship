...
#
# Description :
# Generates detailed phishing analysis reports and organizes
# separate reports into folders according to their risk level.
# Also provides the common report-content writer used by
...
# Write the detailed analysis results into an open report file
...
# Generate and save a separate report based on the email's risk level
def generate_report(
        high_risk_keywords,
    ...
    # Select the report subfolder according to the final risk level
    if level == "SAFE":
        folder = "Safe"
    ...
    # Create a unique report ID and filename for the new report
    report_count = len([
    ...
    report_id = (
        f"{current_time.strftime('%y_%m_%d')}_"
        f"{report_count + 1:03}"
    )

    file_name_txt = (
        f"{current_time.strftime('Phishing_Analysis_Report_%y_%m_%d')}_"
        f"{report_count + 1:03}.txt"
    )
